...
def save_learning_metrics():
    ...
    if type_metric == "all":
        ...
        metrics = [type_metric]
    if type_metric == "all" and do_plot:
        nrows = len(metrics)
        figsize = (5 * ncols, 10)
    ...
    for j, nb_turns in enumerate(nb_turns_list):
        ...
        for ind_prefix, prefix in enumerate(prefix_list):
            ...
            filename = f"results/{circuit_prefix}" \
                               f"{str_dataset}" \
                               f"_{prefix}" \
                               f"{str_nb_turns}" \
                               f"{str_num_q}"
            listing_filename = filename + ".listing"
            params_filename = filename + "_params.json"
            ...
            for i, type_metric in enumerate(metrics):
                ...
                if len(val_list) > 0:
                    metrics_dico[type_metric] = val_list
                else:
                    metrics_dico[type_metric] = []

            df = get_learning_metrics_from_dict(metrics_dico)
            df['init'] = prefix

            with open(params_filename, "rb") as parameters_file:
                params = json.load(parameters_file)

            df['num_layers'] = int(params.get('num_layers', 2))
            df['num_q'] = int(params.get('num_q', num_q))
            df['circuit'] = params.get('layer_name', circuit_prefix)
        ...
        if dfs:
            df = pd.concat(dfs)
            if dataset_name == "":
                dataset = "spirals" + str_nb_turns
            else:
                dataset = dataset_name + str_nb_turns
            df['dataset'] = dataset

            df.set_index('init', inplace=True)

            df['achieve_cvg'] = df['auc'] >= df['auc'].max() * 0.95

            df['auc_first'] = \
                df[df['achieve_cvg']]['auc_iter'] == \
                df[df['achieve_cvg']]['auc_iter'].min()
            df['train_loss_first'] = \
                df[df['achieve_cvg']]['train_loss_iter'] == \
                df[df['achieve_cvg']]['train_loss_iter'].min()
            df['val_loss_first'] = \
                df[df['achieve_cvg']]['val_loss_iter'] == \
                df[df['achieve_cvg']]['val_loss_iter'].min()
            df['first_to_cvg'] = df['auc_first'] + \
                                 df['train_loss_first'] + \
                                 df['val_loss_first']

            df.loc['mean'] = df.mean()
            df = df.astype({'achieve_cvg': 'bool',
                            'train_loss_first': 'bool',
                            'val_loss_first': 'bool',
                            'auc_first': 'bool',
                            'first_to_cvg': 'bool'})

            df.loc['best'] = df.min()
            df.loc['best', 'auc'] = df['auc'].max()
            ...
            df.to_csv('metrics.csv', mode=mode, header=header)
            df_ = pd.read_csv('metrics.csv', sep=",")
            df_.drop_duplicates(inplace=True)
            df_.set_index('init', inplace=True)

            df_.to_csv('metrics.csv')
...
def get_learning_metrics_from_dict(dico):

    result_dict = {}
    for key, val_list in dico.items():
        converging_val = np.mean(val_list[-20:])
        if key == "auc":
            first_occurence = np.argmax(np.array(val_list) >= converging_val*0.98)
        elif "loss" in key:
            first_occurence = np.argmax(np.array(val_list) <= converging_val * 1.02)
        else: # "grad"
            first_occurence = 0
        result_dict[key] = [converging_val]
        result_dict[key+"_iter"] = [first_occurence]
