...
def get_gaussians_dataset(num_samples=50, holdout=20):
    ...
    """
    samples = np.random.multivariate_normal([5, 5], [[1, 0], [0, 1]], num_samples)
    samples2 = np.random.multivariate_normal([-5, -5], [[1, 0], [0, 1]], num_samples)
    samples3 = np.random.multivariate_normal([-5, 5], [[1, 0], [0, 1]], num_samples)
    samples4 = np.random.multivariate_normal([5, -5], [[1, 0], [0, 1]], num_samples)
...
if __name__ == "__main__":
    ...
    dataset = qnn.get_dataset_from_numpy(x_train, y_train, x_val=x_val, y_val=y_val)
